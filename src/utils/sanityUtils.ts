import { SanityTekstObjekt, SanityChildren, SanityDropdown, SanityTekst } from '../typer/sanity';
...
export const sanityBlocktekstToHtml = (tekst: SanityTekstObjekt): string[] => {
    ...
    tekst.tekst.forEach((tekstElement) => {
        tekstElement.children.forEach((child, childIndeks) => {
            const erChildFlettefeltReferanse = erFlettefeltReferanse(tekstElement, child);
...
const erFlettefeltReferanse = (tekstElement: SanityTekst, child: SanityChildren): boolean => {
...
const erTabellReferanse = (tekstElement: SanityTekst, child: SanityChildren): boolean => {
    if (child.marks.length === 1 && !child.marks.includes('strong')) {
        for (let i = 0; i < tekstElement.markDefs.length; i++) {
...
export const erInnholdSanityDropdown = (innhold: SanityDropdown | SanityTekstObjekt): boolean => {
    return (innhold as SanityDropdown).dropdowntittel !== undefined;
};

export const erInnholdTekstObjekt = (innhold: SanityDropdown | SanityTekstObjekt): boolean => {
